defmodule SponsorsWeb.Webhooks.StripeController do
  ...
  def create(conn, _params) do
    ...
    with [signature] <- get_req_header(conn, "stripe-signature"),
         ...
         {invoice_id, stripe_subscription_id, expires_at, paid} <- invoice_details(event) do
      Invoices.create(invoice_id, stripe_subscription_id, expires_at, paid)
    end
  ...
  defp invoice_details(%{data: %{object: %{id: invoice_id, lines: lines, paid: paid}}}) do
    ...
    {invoice_id, stripe_subscription_id, expires_at, paid}
  end
